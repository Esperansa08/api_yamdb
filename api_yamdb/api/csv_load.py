...
con = sqlite3.connect(r"./api_yamdb/db.sqlite3")
cur = con.cursor()


for csv_file in csv_files:

    with open(f'./api_yamdb/static/data/{csv_file}', 'r', encoding="utf-8",) as fin:
        ...
        cur.executemany(
            f'''INSERT INTO {csv_files.get(csv_file)} ({row1_str}) VALUES (
                {to_add});''', reader)

# cur.execute(
#     f"CREATE TABLE IF NOT EXISTS {csv_files.get(csv_file)} ({row1_str});")
con.commit()
con.close()
